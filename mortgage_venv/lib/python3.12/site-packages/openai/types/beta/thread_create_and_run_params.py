...
class ThreadCreateAndRunParamsBase(TypedDict, total=False):
    ...
    **Important:** when using JSON mode, you **must** also instruct the model to
    ...
    """A set of src that are used by the assistant's tools.

    The src are specific to the type of tool. For example, the
...
class Thread(TypedDict, total=False):
    ...
    """
    A set of src that are made available to the assistant's tools in this
    thread. The src are specific to the type of tool. For example, the
    `code_interpreter` tool requires a list of file IDs, while the `file_search`
    tool requires a list of vector store IDs.
    """
